...
fileT0=gsub('_100.txt','',gsub('G360_PC/ph12/PCs_','',fileT))
UNet=union(unlist(lapply(strsplit(fileT0,'_'),'[',1)),unlist(lapply(strsplit(fileT0,'_'),'[',2)))
Net0=strsplit(fileT0[iii],'_')[[1]]
A=rbind(A,A1)
rm(list=c('A1'))
outfile=gsub('_100.txt','',gsub('G360_PC/ph12/PCs_','',fileT[iii]))
outfold='G360_PC_CBSS_Line_mask_out/'
outfold1='G360_CBSS_Line_mask_out/'
system(paste0('mkdir ',outfold))
system(paste0('mkdir ',outfold1))
...
atlas0=fread('../../../template/Fiber_atlas_100points_FullName_qc_LenCurv.csv',data.table=F)
#ind00=which((atlas0$Net1%in%Net0)|(atlas0$Net2%in%Net0)) #############
ind00=which(!((atlas0$Net1%in%Net0)|(atlas0$Net2%in%Net0))) #############change this to non-coupling network
B_train=B_train[,ind00];B_test=B_test[,ind00];
...
write.table(M,file=paste0(outfold,outfile,'_cor.txt'),quote=F)
A_pred1=t(t(A_pred)*attr(A_train,"scaled:scale")+attr(A_train,"scaled:center"));colnames(A_pred1)=colnames(A)
write.table(A_pred1,file=paste0(outfold,outfile,'_pred.txt'),quote=F)
...
write.table(cb_metrics0,file=paste0(outfold1,outfile,'_metrics.txt'),quote=F)
write.table(A_pred1,file=paste0(outfold1,outfile,'_pred.txt'),quote=F)