      ...
      for ( i=0; i<DP_NBHD_COUNT; ++i )
      {
        ...
        W[sr*l1+sc*l2+tr*l3+tc] = 
         dp_edge_weight( Q1, T1, nsamps1, Q2, T2, nsamps2, dim, 
           tv1[sc], tv1[tc], tv2[sr], tv2[tr], idxv1[sc], idxv2[sr],lambda);
...
double dp_edge_weight(
  ...
  double a, double b, 
  double c, double d,
  int aidx, int cidx, double lambda)
{
  double res = 0.0;
  int Q1idx, Q2idx;
  ...
  // Parameters for gamma(t)
  double p = slope; // p = (d - c) / (b - a)
  double q = c - p * a; // q = c - p * a

  // Coefficients for the penalty integral
  double A = p - 1.0;
  double B = q;

  // Compute the penalty integral over [a, b]
